...
function PolicyTextContent() {
  ...
  const citationNote = searchParams.get('cite');
  ...
  // Always try to fetch the most complete text from the API (separate files),
  // falling back to the embedded full_text in policies.ts
  useEffect(() => {
    if (!id) return;
  ...
  if (!id) {
    return (
      <div className="max-w-content mx-auto px-4 sm:px-6 py-16 sm:py-20 text-center">
        ...
        <Link href="/policy-navigator" className="text-secondary hover:underline">Back to Policy Navigator</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-3 sm:px-6 py-4 sm:py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-tertiary mb-6">
        ...
        <span>/</span>
        <Link href={`/policy-navigator/policy/?id=${id}`} className="hover:text-secondary">{policy.short_title}</Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="border-t border-grey-200 pt-6">
              {fullText === undefined ? (
                ...
                <FullTextViewer
                  ...
                  highlightCitation={citationNote || undefined}
                />
              ) : (
                <div className="py-8">
        ...
        <div className="lg:col-span-1">
          <div className="lg:sticky lg:top-[88px] space-y-4 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:pr-1 lg:-mr-1">
            {/* Tab switcher */}
                  ...
                  {policyConnections.map(conn => {
                    ...
                    return (
                      <div key={conn.id} className="border border-grey-100 rounded-lg p-2.5 hover:border-secondary/30 hover:bg-secondary/5 transition group">
                        ...
                        <Link href={`/policy-navigator/policy-text/?id=${linkedId}`}

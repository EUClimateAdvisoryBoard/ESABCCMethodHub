...
import staticPdfLinks from '@/data/reference-pdf-links.json';
...
interface RefSummary {
  ...
  // so we have to hit the raw Supabase URL directly.  `static` refs come
  // from the bundled library + the open-access link sidecar — their PDFs
  // live on publisher hosts, so they go through /api/references/fetch-pdf
  // which dodges publisher CORS.
  source: 'server' | 'local' | 'static';
}
...
// Bundled static library lookup. PDF URLs come from the sidecar generated by
// scripts/audit-static-references.mjs (open-access copies via Unpaywall).
function loadStaticRefById(id: string): RefSummary | null {
  const found = staticReferences.find(r => r.id === id);
  if (!found) return null;
  const links = (staticPdfLinks.links ?? {}) as Record<string, string>;
  return {
    id: found.id,
...
function AnnotatePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      // The custom store wins over the bundled static library — a few ids
      // (e.g. ref-1961) exist in both, and the custom row may carry an
      // uploaded PDF where the static sidecar has none.
      const staticFallback = loadStaticRefById(id);
      try {
        const resp = await fetch('/api/references/library', { cache: 'no-store' });
        if (!resp.ok) {
          if (staticFallback) setRef(staticFallback);
          else setLoadError('Could not reach the reference library API.');
          setLoaded(true);
          return;
        }
        ...
        if (!found) {
          if (staticFallback) setRef(staticFallback);
          else setLoadError('Reference not found in the library.');
        } else {
          setRef({
            id:       found.id,
            title:    found.title    || 'Untitled',
          ...
          <div className="border border-grey-200 rounded-lg overflow-hidden bg-white">
            {ref.pdfUrl ? (
              <PdfAnnotator
                ...
                // Static-library refs point at publisher-hosted open-access
                // PDFs, which almost never send CORS headers — those go
                // through the generic fetch-pdf proxy instead.  Browser-local
                // refs don't exist on the server, so we hit the raw URL
                // directly (public Supabase buckets allow CORS for GETs).
                // Either way we provide the raw URL as a fallback so the
                // viewer can retry if the primary fails.
                pdfUrl={
                  ref.source === 'server'
                    ? `/api/references/pdf?id=${encodeURIComponent(ref.id)}`
                    : ref.source === 'static'
                      ? `/api/references/fetch-pdf?url=${encodeURIComponent(ref.pdfUrl)}`
                      : ref.pdfUrl
                }

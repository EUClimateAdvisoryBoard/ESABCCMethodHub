...
const KINDS = new Set(['reference', 'policy', 'news', 'scenario_view', 'segment']);
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const body = (await req.json().catch(() => ({}))) as { kind?: string; ref_id?: string; note?: string };
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await sb.from('workspace_items').delete().eq('id', itemId).eq('workspace_id', params.id);

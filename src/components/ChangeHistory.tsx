...
export default function ChangeHistory({ artefactKind, artefactId, className = '' }: Props) {
  ...
  return (
    <span className={`relative inline-block ${className}`}>
      <button
        onClick={() => setOpen(o => !o)}
        className="text-[11px] text-grey-500 hover:text-tertiary dark:hover:text-white underline-offset-2 hover:underline"
        aria-label="Show change history"
      >
        history
      </button>
      {open && (
        <div className="absolute right-0 mt-1 z-40 w-[360px] max-h-[400px] overflow-y-auto bg-white dark:bg-[var(--mh-card)] dark:text-[var(--mh-fg)] border border-grey-200 dark:border-[var(--mh-border)] rounded-lg shadow-xl">
          <div className="px-3 py-2 border-b border-grey-100 dark:border-[var(--mh-border)] flex items-center justify-between">
            <p className="text-[10px] uppercase tracking-wide text-grey-500">Change history · {artefactKind}</p>
            <button onClick={() => setOpen(false)} className="text-grey-500" aria-label="Close">✕</button>
          </div>
                ...
                <li key={e.id} className="px-3 py-2.5 text-xs">
                  ...
                  {changed.length > 0 && (
                    <ul className="mt-1 space-y-0.5">
                      {changed.map(k => (
                        <li key={k} className="font-mono text-[10.5px]">

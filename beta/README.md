...
| `short-formats/`              | Short Formats | Four compact, one-page briefs that re-package the Board's published advice on **ETS**, **LULUCF**, the **energy crisis** and **flexibilities for international carbon mitigation**. Content is drawn *only* from existing ESABCC recommendations (looked up by id from `src/data/esabcc-recommendations.ts`) — nothing is invented — and every card links back to the source report it comes from. |

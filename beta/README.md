...
| `file-tracker/`               | File Tracker         | Per-dossier news timeline: for each relevant EU climate-policy file (LULUCF, EU ETS, ETS2, Effort Sharing, the 2040 target, CBAM, RED III, EPBD, Nature Restoration, CAP…) it shows recent developments in chronological order with short summaries, so a reader can open a file and see "there is this discussion now". Daily refresh from the Commission / Council / EEA / EUR-Lex RSS feeds (`src/lib/rss-feeds.ts`) attributed per file via `src/lib/policy-files.ts`, merged on top of a curated baseline snapshot (`public/data/file-tracker-seed.json`); served by `/api/file-tracker` (revalidates daily). Curated baseline items are illustrative and should be verified against the linked source. |

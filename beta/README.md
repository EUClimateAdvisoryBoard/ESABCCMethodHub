...
| `national-climate-policies/`  | National Level Climate Policies | EU-27 laws & policies from climate-laws.org (CC-BY 4.0); in-module refresh via Climate Policy Radar API + Supabase snapshot. Includes a member-state assessment & best-practice section focused on the states with an independent climate council (the ESABCC's peers), scored live on five documented policy-architecture dimensions — see `analysis/MEMBER-STATE-BEST-PRACTICES.md`. |

  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[640px] border-separate" style={{ borderSpacing: 0 }}>
        <thead>
          <tr>
            <th className="pb-2 pr-3 text-left text-[10px] font-semibold uppercase tracking-wide text-tertiary">
        ...
        <tbody>
          {CLEANTECH_INDUSTRIES.map((t) => {
            ...
            return (
              <tr key={t.id}>
                <td className="border-t border-grey-100 py-1 pr-3">
                  <button
                    type="button"
                    ...
                    className={`flex w-full items-center gap-2 rounded-lg border px-2 py-1.5 text-left transition ${
                      isSel
                        ...
                        : 'border-transparent hover:border-grey-200 hover:bg-grey-50'
                    }`}
                  >
                    <span
                    ...
                    <span className="min-w-0">
                      <span className="block truncate text-[12.5px] font-semibold text-grey-900">{t.name}</span>
                      <span className="block truncate font-mono text-[9.5px] text-grey-500">
                        NACE {t.nace.join(' · ')}
                      </span>
                    </span>
                  </button>
                ...
                {DEMAND_SECTORS.map((s) => {
                  ...
                  return (
                    <td key={s.id} className="border-t border-grey-100 py-1 text-center align-middle">
                      {cell ? (
                        <button
                          type="button"
                          onClick={() => onSelect(t.id)}
                        ...
                        >
                          <span
                            className={`inline-block h-4 w-4 rounded-full border-2 transition group-hover:scale-125 ${
                              cell.role === 'primary' ? '' : 'bg-white'
                            }`}
                            style={{
                              borderColor: s.color,
                              background: cell.role === 'primary' ? s.color : undefined,
                            }}
                            aria-label={`${cell.role} support`}
                          />
                        </button>
                      ) : (
                        <span className="inline-block h-1 w-1 rounded-full bg-grey-200" aria-hidden />
                      )}
                    </td>
                  );
                })}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10.5px] text-grey-500">
        ...
        <span className="inline-flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded-full border-2 border-grey-500 bg-white" /> supporting / enabling role
        </span>
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <div className="space-y-3">
        <div>
          <div className="mb-1 flex flex-wrap items-center gap-1.5">
            ...
            {tech.nace.map((c) => (
              <span
                key={c}
                className="rounded-full border border-grey-300 bg-grey-50 px-1.5 py-0.5 font-mono text-[9px] font-semibold text-tertiary"
              >
                {c}
              </span>
            ))}
          </div>
          <p className="text-[12px] leading-relaxed text-grey-600">{tech.what}</p>
          {tech.naceNote ? <p className="mt-1 text-[10px] leading-snug text-grey-400">{tech.naceNote}</p> : null}
        </div>
        <div className="rounded-lg bg-grey-50 p-2.5">
          ...
          <ul className="space-y-1">
            {tech.supports.map((c) => (
              <li key={c.sectorId} className="text-[11.5px] leading-snug text-grey-800">
                <span
                ...
                <span className="text-grey-500"> ({c.role})</span> — {c.note} <SourceLink source={c.source} />
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="space-y-3">
        <div className="rounded-lg border border-grey-200 bg-white p-2.5">
          <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wide text-tertiary">
...
function PathwayChart({
  ...
  return (
    <div>
        ...
        >
          ...
          {[0, 25, 50, 75, 100].map((pct) => (
            <g key={pct}>
              ...
              />
              <text x={CHART.padL - 6} y={y(pct) + 3.5} textAnchor="end" fontSize={10} fill="#94A3B8">
                −{pct}%
              </text>
            </g>
          ))}
                ...
                {p.points.map((pt, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <text x={bx + barW / 2} y={by - 4} textAnchor="middle" fontSize={9.5} fontWeight={700} fill="#334155">
                        −{pt.pct}%
                      </text>
                      <text
                        x={bx + barW / 2}
      ...
      {/* hover / source strip */}
      <div className="mt-1 min-h-[34px] rounded-lg bg-grey-50 px-3 py-2 text-[11.5px] leading-snug text-grey-700">
        {hover ? (
          <>
...
function PriorityCards() {
  ...
  return (
    <div>
      <div className="mb-2 rounded-lg border border-accent-orange bg-surface-orange p-2.5 text-[11px] leading-snug text-grey-700">
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        {EXTERNAL_PRIORITIES.map((p) => {
          ...
          if (!t) return null;
          return (
            <div key={p.techId} className="rounded-lg border border-grey-200 bg-white p-3">
              <div className="flex items-center gap-2">
                <span className="flex h-6 w-6 flex-none items-center justify-center rounded-full bg-grey-900 text-[11px] font-bold text-white">
                  {p.rank}
                </span>
                <span
                  className="flex h-6 w-6 flex-none items-center justify-center rounded-md text-[13px] text-white"
                ...
                <span className="text-[13px] font-bold text-grey-900">{t.name}</span>
              </div>
              <p className="mt-2 text-[11.5px] leading-snug text-grey-700">
                <span className="font-semibold text-grey-900">Mitigation leverage: </span>
                {p.rationale}
              </p>
              <p className="mt-1.5 text-[11.5px] leading-snug text-grey-700">
                <span className="font-semibold text-grey-900">Competitiveness read: </span>
                {p.competitiveness}
              </p>
            </div>
          );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {EXTERNAL_HEADLINES.map((f, i) => (
          <div key={i} className="rounded-lg border border-grey-200 bg-white p-3">
            <div className="text-[15px] font-bold leading-tight text-grey-900">{f.value}</div>
            <p className="mt-1 text-[11px] leading-snug text-grey-600">{f.note}</p>
            <div className="mt-1">
      ...
      {/* 1 · who supports whom */}
      <section>
        <h3 className="text-sm font-bold uppercase tracking-wide text-grey-900">
          1 · Who supports whom — the clean-tech industries and the sectors they decarbonise
        </h3>
        <p className="mb-3 mt-1 max-w-text text-[12px] leading-relaxed text-grey-600">
      ...
      {/* 2 · industry detail */}
      <section id="external-industry-detail" className="rounded-xl border border-grey-200 bg-grey-50 p-4">
        <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-grey-900">
      ...
      <section>
        ...
        <p className="mt-2 text-[10.5px] leading-snug text-grey-500">{PATHWAY_CAVEAT}</p>
      </section>

      {/* 4 · priority read */}
      <section>
        <h3 className="mb-2 text-sm font-bold uppercase tracking-wide text-grey-900">

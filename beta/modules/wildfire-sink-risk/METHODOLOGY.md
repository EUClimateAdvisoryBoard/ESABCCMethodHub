...
back to an embedded snapshot of the same payload (`FALLBACK_SEASON` in
`live.ts`, currently taken 2026-08-05, week 30) and labels itself accordingly.

In-season EFFIS figures are provisional and revised as new fires are mapped,
and the revisions run **in both directions** — the 5 August 2026 refresh moved
weeks 28 and 29 down while week 30 rose by 28,693 ha. Treat any single week's
figure as a current best mapping, not a settled total; see
`docs-internal/wildfire-effis-source-refresh-2026-08-05.md`.
...
series fetched from the open Copernicus API on 2026-08-05, plus Annex IIa of
...
The EFFIS parts of that file are regenerated by
`scripts/refresh-wildfire-effis-data.mjs`, which rewrites only the
machine-derived arrays and leaves the Annex IIa figures — which have no API
source — untouched. Re-run it to pick up EFFIS remapping; the current-year row
is the season to date and is provisional. Refreshes are recorded in
`docs-internal/`, most recently
`wildfire-effis-source-refresh-2026-08-05.md`.
...
| `scripts/refresh-wildfire-effis-data.mjs` | Regenerates the EFFIS arrays in `analyses/data.ts` |

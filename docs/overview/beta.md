...
| M·53  | `policy-target-flows/`       | Policy Targets → Indicators | Every relevant M·36 target on a sector flow chart in the Policy Gap 2.0 style, matched to the series, dataset or compliance milestone that measures it — opening on a one-figure overview of all nine sector columns, each expandable into its chart — no target left unassessed, and the measurement gaps ranked by the dataset that would close them. |
